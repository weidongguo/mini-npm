...
class Registry {
  ...
  async getPackageAndItsDependenciesDownloadURLs(name, versionRange, outputURLs) {
    const { normalizedName, normalizedVersionRange } = Utils.normalizeVersionRange(name, versionRange);
    const version = await this.#getPackageMaxSatisfyingVersion(normalizedName, normalizedVersionRange)
    if(!version) {
      throw `No matching version found for ${versionRange}`;
    }

    const fullyQualifiedPackage = `${normalizedName}@${version}`
    if(outputURLs[fullyQualifiedPackage]) {
      // if it has already been processed, don't recurse further to avoid a cyclic reference
      return;
    }

    const response = await fetch(`${this.#npmRegistry}/${normalizedName}/${version}`)
    const metadata = await response.json();

    if(!metadata["dist"]) {
      throw `Failed to get download URL for package ${fullyQualifiedPackage}`
    }
