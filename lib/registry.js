...
class Registry {
   ...
   * @param {*} versionRange e.g. ^1.1.0 means keep major version, ~1.1.0 means keeps major & minor version
   * @returns The satisfying version
   */
  ...
  async getPackageAndItsDependenciesDownloadURLs(name, versionRange, outputURLs) {
    ...
    if(outputURLs[fullyQualifiedPackage]) {
      ...
      console.warn(`Cylic reference detected for ${fullyQualifiedPackage}. Skipped`);
      return;
    }
    console.log(`${name}@${versionRange}\t-----resolves-to----->\t${fullyQualifiedPackage}`);
    ...
    outputURLs[fullyQualifiedPackage] = { url, shasum };
