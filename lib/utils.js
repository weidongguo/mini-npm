...
export default class Utils {
  ...
  /**
   * Occasionally, the package and versionRange is formatted in a way that the specific package
   * shall be fetched under a different name.
   * e.g. { "wrap-ansi-cjs": "npm:wrap-ansi@^7.0.0" } means that we should fetch wrap-ansi from npm registry with version ^7.0.0.
   * This would be the right package for wrap-ansi-cjs. Essentially, this particular version is intended for CommonJS (cjs) environment
   *
   * @param {string} versionRange 
   * @returns {string} normalized version range, so make sure no prefix
   */
  static normalizeVersionRange(name, versionRange) {
    const tokens = versionRange.split('@');
    if (tokens.length > 1) {
      const normalizedVersionRange = tokens[1]

      const prefix = tokens[0];
      const sourceAndName = prefix.split(":");
      const normalizedName = sourceAndName[sourceAndName.length-1]
    ...
    if (tokens.length == 1) {
      return {
        normalizedName: name,
        normalizedVersionRange: versionRange
      }
    }
  }
}